...
@property (nonatomic, strong) JuBaoTableAlertNumberOfRowsBlock numberOfRows;
@property (nonatomic, strong) JuBaoTableAlertTableCellsBlock cells;
...
@implementation JuBaoTableAlert

#pragma mark - JuBaoTableAlert Class Method
...
#pragma mark - JuBaoTableAlert Initialization

-(id)initWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle numberOfRows:(JuBaoTableAlertNumberOfRowsBlock)rowsBlock andCells:(JuBaoTableAlertTableCellsBlock)cellsBlock
{
...
-(void)show
{
	...
	self.cancelButton.frame = CGRectMake(kLateralInset + (kTableAlertWidth - kLateralInset * 3)/2 + kLateralInset, self.table.frame.origin.y + self.table.frame.size.height + kCancelButtonMargin, (kTableAlertWidth - kLateralInset * 3)/2, kCancelButtonHeight);
	...
	[self.cancelButton setBackgroundImage:[[UIImage imageNamed:@"MLTableAlertCancalButton.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:0] forState:UIControlStateNormal];
	[self.cancelButton setBackgroundImage:[[UIImage imageNamed:@"MLTableAlertCancalButton.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:0] forState:UIControlStateHighlighted];
    ...
    [self.alertBg addSubview:_cancelButton];
    _okButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
	_okButton.frame = CGRectMake(kLateralInset, self.table.frame.origin.y + self.table.frame.size.height + kCancelButtonMargin, (kTableAlertWidth - kLateralInset * 3)/2, kCancelButtonHeight);
	_okButton.titleLabel.textAlignment = NSTextAlignmentCenter;
	_okButton.titleLabel.font = [UIFont boldSystemFontOfSize:17.0];
	[_okButton setTitle:@"确定" forState:UIControlStateNormal];
	[_okButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
	[_okButton setBackgroundColor:[UIColor clearColor]];
	[_okButton setBackgroundImage:[[UIImage imageNamed:@"MLTableAlertButton.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:0] forState:UIControlStateNormal];
	[_okButton setBackgroundImage:[[UIImage imageNamed:@"MLTableAlertButtonPressed.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:0] forState:UIControlStateHighlighted];
	_okButton.opaque = NO;
	_okButton.layer.cornerRadius = 5.0;
    
	[self.alertBg addSubview:_okButton];
