...
+ (void)showMsg:(UIView *)view title:(NSString *)title delay:(float)delay
{
    MBProgressHUD *tmpH = [[MBProgressHUD alloc] initWithView:view];
    if (kVersion >= 7.0) {
        tmpH.labelColor = [UIColor blackColor];
    }
    [tmpH updateIndicators];
    [view addSubview:tmpH];
    [view bringSubviewToFront:tmpH];
    [tmpH setAnimationType:MBProgressHUDAnimationZoom];
    tmpH.labelText = title;
    [tmpH setMode:MBProgressHUDModeText];
    [tmpH show:YES];
    [tmpH hide:YES afterDelay:delay];
}

+ (void)showMsg:(UIView *)view title:(NSString *)title detail:(NSString *)detail delay:(float)delay
{
    MBProgressHUD *tmpH = [[MBProgressHUD alloc] initWithView:view];
    [view addSubview:tmpH];
    [tmpH setAnimationType:MBProgressHUDAnimationZoom];
    tmpH.labelText = title;
    tmpH.detailsLabelText = detail;
    [tmpH setMode:MBProgressHUDModeText];
    [tmpH updateIndicators];
    [tmpH show:YES];
    [tmpH hide:YES afterDelay:delay];
